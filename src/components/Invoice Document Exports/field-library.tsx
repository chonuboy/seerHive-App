...
export default function FieldLibraryPreview({
  currentTemplateId,
  ...
  const removeAttribute = async (attributeId: number) => {
    try {
      ...
      if (response && !response.error) {
        setFieldAttributes((prev) =>
          prev.filter((attr) => attr.fieldAttributeId !== attributeId)
        );
      } else {
  ...
  const handleCreateMapping = async () => {
    ...
    try {
      const reqData = {
        ...
        maxLength: newMapping.maxLength,
        decimalPlaces: newMapping.decimalPlaces,
        dropdownOptions: newMapping.dropdownOptions || "",
        validationRegex: newMapping.validationRegex || "",
        tooltipText: newMapping.tooltipText || "",
      };

      console.log("Creating mapping with data:", reqData);

      const response = await createTemplateFieldMap(reqData);
      console.log("Create mapping response:", response);

      if (response && !response.error) {
        const createdMapping: TemplateFieldMap = response;

        const updatedMappings = [...templateMappings, createdMapping];
  ...
  const handleDeleteMapping = async (mappingId: number) => {
    if (
      !confirm("Are you sure you want to remove this field from the template?")
    ) {
      return;
    }
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <FieldIcon className="h-4 w-4 text-blue-600 mt-1" />
                      <div className="flex-1">
