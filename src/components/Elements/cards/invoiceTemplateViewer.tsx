...
import { Check, Plus, Loader2, X, EyeIcon, Edit } from "lucide-react";
import { useState, useEffect, useCallback, Fragment } from "react";
import { Dialog, Transition } from "@headlessui/react";
import {
  fetchAllInvoiceTemplates,
  createInvoiceTemplate,
} from "../../../api/invoice/invoicetemplate";
import { fetchAllBillingTypes } from "../../../api/invoice/billing-type";
import { fetchAllTemplates } from "../../../api/invoice/templatestyling";
import InvoicePreviewer from "../utils/invoicePreviewer";
...
export default function TemplateViewer() {
  ...
  const handleEditTemplate = (templateId: number) => {
    router.push(`/invoice/configure/${templateId}`);
  };
  ...
  const handleAddNewTemplate = async () => {
    ...
    try {
      const selectedBillingType = billingTypes.find(
        (bt) => bt.billingTypeId === newTemplate.billingTypeId
      );
      const selectedTheme = themes.find(
        (t) => t.themeName === newTemplate.themeName
      );
      ...
      const templateData = {
        ...
        description:
          ...
          `Custom ${selectedBillingType?.billingTypeName || "template"}`,
  ...
  return (
    <div className="space-y-4 px-2">
      {/* Invoice Previewer */}
      <InvoicePreviewer
        templateId={previewTemplateId || 1}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-2 py-1 hover:bg-cyan-600 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex gap-2 items-center bg-cyan-500 text-white"
          >
            Create New
            <Plus className="h-4 w-4" />
          </button>
      ...
      <div className="grid grid-cols-4 gap-6">
        {templates.map((template) => (
          <div
            ...
            className={`bg-white rounded-lg p-4 cursor-pointer transition-all ${"border-2 border-gray-200 hover:border-gray-300 hover:shadow-sm"}`}
          >
            <div className="flex items-center gap-2 justify-end">
              <button
                className="hover:text-blue-600 transition-colors"
                onClick={() => handlePreviewTemplate(template.templateId)}
                title="Preview Template"
              >
                <EyeIcon className="text-gray-400 hover:text-blue-600" />
              </button>
              <button
                ...
                title="Edit Template"
              >
                <Edit className="w-5 h-5 text-gray-400 hover:text-green-600" />
              </button>
            </div>
                ...
                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mb-4">
                    ...
                    <Dialog.Description className="text-sm text-gray-500">
                      Configure your new invoice template with name, billing
                      type, and theme.
                    </Dialog.Description>
                  </div>
                      ...
                      <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        >
                          ...
                          {billingTypes.map((billingType) => (
                            <option
                              key={billingType.billingTypeId}
                              value={billingType.billingTypeId.toString()}
                            >
                            ...
                            <div className="p-3 border border-gray-300 rounded-lg">
                              ...
                              <div className="flex gap-2">
                                ...
                                <div
                                  ...
                                  style={{
                                    backgroundColor:
                                      getSelectedTheme()!.backgroundColor,
                                  }}
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    ...
                    <button
                      ...
                      disabled={
                        !newTemplate.templateName?.trim() || loading.creating
                      }
