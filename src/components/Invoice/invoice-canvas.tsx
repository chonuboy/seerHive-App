...
import { fetchTemplateFieldMap, createTemplateFieldMap } from "../../api/invoice/templatefieldmap";
import {
...
export default function InvoiceCanvas({
  ...
  // NEW FUNCTION: Create field mapping via API
  const createFieldMapping = async (
    ...
    try {
      // Extract section ID from the section string (e.g., "section-2" -> 2)
      const sectionIdNumber = parseInt(sectionId.replace('section-', ''));
      
      // Calculate display order based on position
      const section = sections.find(s => s.id === sectionId);
      const gridColumnsCount = section ? getGridColumnsCount(section.layoutConfig.gridColumns) : 2;
      const displayOrder = (row * gridColumnsCount) + col + 1;
      ...
      const reqData = {
        ...
        invoiceFieldMaster: {
          invoiceFieldId: fieldData.fieldId || fieldData.invoiceFieldMaster?.invoiceFieldId,
        },
        invoiceSectionMaster: {
          sectionId: sectionIdNumber,
        },
        displayLabel: fieldData.displayLabel || fieldData.name,
        controlType: fieldData.controlType || "text",
        ...
        decimalPlaces: fieldData.decimalPlaces || null,
        dropdownOptions: fieldData.dropdownOptions || null,
        validationRegex: fieldData.validationRegex || null,
        tooltipText: fieldData.tooltipText || null,
      };

      console.log("Creating field mapping with data:", reqData);

      const response = await createTemplateFieldMap(reqData);
      console.log("Field mapping API response:", response);
  ...
  const handleDrop = async (
    ...
    try {
      ...
      // Check if this is a field from the library (has isMapped flag)
      if (fieldData.isMapped === false) {
        // This is a field from the library that needs to be mapped to the template
        console.log("Creating field mapping for dropped field:", fieldData);
        
        const apiResponse = await createFieldMapping(fieldData, section, row, col);
        
        if (apiResponse) {
          // Field mapping created successfully, reload the template mappings
          await loadTemplateFieldMaps(currentTemplateId!);
          return; // Exit early since we're reloading the data
        } else {
          console.error("Failed to create field mapping via API");
  ...
  const removeField = async (fieldId: string) => {
    ...
    // If this field has a templateFieldMapId, we should also delete it from the API
    if (fieldToRemove?.fieldData?.templateFieldMapId && currentTemplateId) {
      // You would need to implement deleteTemplateFieldMap API call here
      console.log("Should delete field mapping with ID:", fieldToRemove.fieldData.templateFieldMapId);
      // await deleteTemplateFieldMap(fieldToRemove.fieldData.templateFieldMapId);
      // await loadTemplateFieldMaps(currentTemplateId);
      // return; // Exit early since we're reloading data
    }
